 ...
 * the superset is the right operator signal (which subtrees are withheld, how
 * big). The message says "chunked page(s)", never "searchable".
 *
 * Status (CV-1a): pages hidden ONLY under DEFAULT excludes → `ok` (those are
 * intentional noise; warning would make every healthy brain look unhealthy).
 * Pages hidden under a NON-default (env-supplied) prefix → `warn` (the operator
 * added that exclude; surfacing it is the signal they want). The message is
 * agent-prescriptive: the consumer can move content out of the excluded prefix
 * or pass `include_slug_prefixes` on the query.
 *
 * NOTE: this does NOT verify `archive/` pages are embedded/graphed — after the
 * #1777 fix `archive/` is no longer excluded, so it never appears here. Archive
 * findability is verified by the search tests + `gbrain search` manually.
 */
export async function checkHiddenBySearchPolicy(engine: BrainEngine): Promise<Check> {
  ...
  try {
    ...
    // an explicit ESCAPE clause. The candidate gate is EXISTS(content_chunks);
    ...
    // Non-default (env/caller-supplied) prefix hiding pages → warn; the operator
    // added that exclude on purpose and wants to see its effect.
...
export async function buildChecks(
  ...
  if (engine !== null) {
    ...
    checks.push(await checkSearchMode(engine));
    // issue #1777 — hidden_by_search_policy: chunked pages withheld from default
    // search by the hard-exclude prefix policy (search-policy adjacent).
